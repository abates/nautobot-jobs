...
def docker_compose(context, command, **kwargs):
    ...
    for compose_file in context.nautobot_jobs.compose_files:
        compose_file_path = os.path.join(
            context.nautobot_jobs.compose_dir, compose_file
        )
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_jobs.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def pylint(context):
    ...
    migrations_dir = (
        Path(__file__).absolute().parent / Path("nautobot_jobs") / Path("migrations")
    )
